...
void vmem_init() {
    ...
    for (uintptr_t virt = 0; virt < 0x400000; virt += PAGE_SIZE) {  /// might need to change this upper bound
...
void vmem_map_page(uintptr_t virt_addr, uintptr_t phys_addr, uint32_t flags) {
    ...
    // map the page
    page_table[table_index] = (phys_addr & 0xFFFFF000) | (flags & 0xFFF) | PAGE_PRESENT;
